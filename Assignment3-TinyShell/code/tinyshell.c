...
// @@@@@@@@@@@    //'''                                                      '''
    ...
    else if (fc == 0)
    {
      ...
      if (C.redirection == 1)
      {
         ...
         */
        fd = open(C.ofile, O_RDWR | O_CREAT | O_TRUNC, S_IRWXU);
        if (fd < 0)
        {
          PRINT_ERROR_SYSCALL("redirection");
          continue;
        }
      ...
      if (fileno(stdout) != o_stdout)
        {
          dup2(o_stdout, fileno(stdout));
          close(fd);
        }
